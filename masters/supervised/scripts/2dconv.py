...
# Data processing #
###################
data = scipy.io.loadmat('data/2dconv.mat',struct_as_record=False)
learn = data['l_data']
control_old = data['n_data']

_,control_new = read_zip('data/Control2DConv_88N.zip')
...
if not use_sub:
    ...
    pylab.axes([0.1, 0.0+bottom_pad, 0.33, 0.5-v_pad])
...
if use_sub:
    pylab.text(152.0,baseline-0.05,"3 layer control",figure=f,fontsize=12)
    pylab.text(152.0,baseline_new-0.05,"2 layer control",figure=f,fontsize=12)
#pylab.axis([xmin, xmax, ymin, ymax])
pylab.axis([0.0, 150.0, 0.9, 2.5])

pylab.ylabel(r"Relative error (learning vs. analytic)",ha='center')    
pylab.xlabel(r"Learning time (seconds)")
pylab.title(r"$f(\mathbf{x}) = \left[x_1,x_2\right] \otimes \left[x_3,x_4\right]$",fontsize=16)

if save and not use_sub:
    pylab.savefig('2dconv.pdf')
