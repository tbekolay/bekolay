...
# Data processing #
###################
_,control = read_zip('data/ControlX1X2X3_84N.zip')
time,learn = read_zip('data/x1x2x3.zip')
baseline = 1
...
if not use_sub:
    ...
    pylab.axes([0.66+h_pad, 0.5+bottom_pad, 0.33-h_pad, 0.5-v_pad])
...
pylab.axis([0.0, 100.0, 0.9, 1.7])

if not use_sub:
    pylab.ylabel(r"Relative error (learning vs. analytic)",ha='center')
if not use_sub:
    pylab.xlabel(r"Learning time (seconds)")
pylab.title(r"$f(\mathbf{x}) = \left[x_1 x_2, x_1 x_3, x_2 x_3 \right]$",fontsize=16)

if save and not use_sub:
    pylab.savefig('x1x2x3.pdf')
