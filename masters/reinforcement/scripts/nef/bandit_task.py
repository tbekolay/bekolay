...
def build_network():
    ...
    cortex_gate = net.make('CortexGate',1,1,encoders=[[1.0]],intercept=[0.0],max_rate=(100,100))
    net.connect(experiment.getOrigin('cortex_gate'),cortex_gate)
    net.connect(cortex_gate,cortex,weight_func=gate_weights)
    
    thalamus = net.make('Thalamus',N * D,D)
    net.connect(thalamus,experiment.getTermination('thalamus'))
    
    nps.basalganglia.make_basal_ganglia(net,cortex,thalamus,D,N,learn=True)
    StrD1 = net.network.getNode('StrD1')
    StrD2 = net.network.getNode('StrD2')
    
    vStr = net.make('Ventral Striatum',N*D*2,D*2)
    net.connect(StrD1.getOrigin('X'),vStr,index_post=[0,1])
    ...
    net.learn_array(StrD1,'Cortex','Ventral Striatum',stdp=False,rate=1e-7)
    net.learn_array(StrD2,'Cortex','Ventral Striatum',stdp=False,rate=1e-7)
    
    vStr_gate = net.make('vStrGate',1,1,encoders=[[1.0]],intercept=[0.0],max_rate=(100,100))
